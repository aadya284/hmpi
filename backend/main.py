...
def create_visualization(results: Dict, filename: str) -> str:
    """Create visualization charts and return base64 encoded image"""
    # Set style
    plt.style.use('seaborn-v0_8')
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle(f'HMPI Analysis Report - {filename}', fontsize=16, fontweight='bold')
    
    # 1. HMPI Value and Risk Category
    ax1.bar(['HMPI Score'], [results['hmpi']], color=results['risk_category']['color'])
    ax1.set_title('HMPI Score and Risk Level')
    ax1.set_ylabel('HMPI Value')
    ax1.text(0, results['hmpi'] + 2, f"{results['hmpi']:.2f}", ha='center', fontweight='bold')
    ax1.text(0, results['hmpi'] - 5, results['risk_category']['category'], ha='center', 
             fontweight='bold', color=results['risk_category']['color'])
    
    # 2. Metal Contributions
    metals = list(results['metal_contributions'].keys())
    contributions = [results['metal_contributions'][metal]['contribution'] for metal in metals]
    colors = ['#ff6b6b', '#4ecdc4', '#45b7d1', '#96ceb4', '#feca57', '#ff9ff3', '#54a0ff']
    
    ax2.pie(contributions, labels=metals, autopct='%1.1f%%', colors=colors[:len(metals)])
    ax2.set_title('Metal Contribution to HMPI')
    
    # 3. Hazard Quotients
    hq_metals = list(results['hazard_quotients'].keys())
    hq_values = [results['hazard_quotients'][metal]['hazard_quotient'] for metal in hq_metals]
    
    bars = ax3.bar(range(len(hq_metals)), hq_values, color=['#ef4444' if hq > 1 else '#22c55e' for hq in hq_values])
    ax3.set_title('Hazard Quotients by Metal')
    ax3.set_ylabel('HQ Value')
    ax3.set_xticks(range(len(hq_metals)))
    ax3.set_xticklabels(hq_metals, rotation=45, ha='right')
    ax3.axhline(y=1, color='red', linestyle='--', alpha=0.7, label='Safe Limit (HQ=1)')
    ax3.legend()
    
    # 4. Quality Ratios
    ratio_metals = list(results['quality_ratios'].keys())
    ratio_values = [results['quality_ratios'][metal]['ratio'] for metal in ratio_metals]
    
    bars = ax4.bar(range(len(ratio_metals)), ratio_values, 
                   color=['#ef4444' if ratio > 1 else '#22c55e' for ratio in ratio_values])
    ax4.set_title('Concentration to Standard Limit Ratios')
    ax4.set_ylabel('Ratio (Concentration/Standard)')
    ax4.set_xticks(range(len(ratio_metals)))
    ax4.set_xticklabels(ratio_metals, rotation=45, ha='right')
    ax4.axhline(y=1, color='red', linestyle='--', alpha=0.7, label='Standard Limit')
    ax4.legend()
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            if concentrations:
                ...
                try:
                    chart_base64 = create_visualization(sample_results, f"Sample_{index + 1}")
                    sample_info['visualization'] = f"data:image/png;base64,{chart_base64}"
                except Exception as e:
                    sample_info['visualization'] = None
                    sample_info['chart_error'] = str(e)
        ...
        overall_stats = {
            ...
            'average_hmpi': np.mean(hmpi_values),
            'min_hmpi': np.min(hmpi_values),
            'max_hmpi': np.max(hmpi_values),
            'std_hmpi': np.std(hmpi_values),
            'risk_distribution': {}
        }
...
async def calculate_hmpi_direct(concentrations: Dict[str, float]):
    ...
    try:
        ...
        # Create visualization
        try:
            chart_base64 = create_visualization(results, "Direct_Calculation")
            results['visualization'] = f"data:image/png;base64,{chart_base64}"
        except Exception as e:
